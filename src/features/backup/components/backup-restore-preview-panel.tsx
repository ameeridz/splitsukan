...
import type { BackupSummary, SplitsukanBackupFile } from "../backup-model";
import {
...
export function BackupRestorePreviewPanel() {
  ...
  function confirmRestore() {
    ...
    const restoredCount = selectedBackup.summary.sessionCount;
    restoreBackupSessions(selectedBackup.backup.sessions);
    setRestoreSuccess(
      `Restore completed successfully. ${restoredCount} ${
        restoredCount === 1 ? "session was" : "sessions were"
      } restored.`,
    );
    setSelectedBackup(null);
  ...
  const metrics = selectedBackup
    ? [
        [FileJson, selectedBackup.summary.sessionCount, "sessions"],
        [UsersRound, selectedBackup.summary.participantCount, "participants"],
        [WalletCards, selectedBackup.summary.expenseCount, "expenses"],
        [ReceiptText, selectedBackup.summary.repaymentCount, "repayments"],
      ] as const
    : [];
      ...
      <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
        <div className="max-w-2xl">
          ...
          <p className="mt-2 text-sm leading-6 text-muted-foreground">
            ...
            previewed before any current local data is replaced.
          </p>
        </div>
        ...
        <div className="shrink-0">
          ...
          <label
            ...
            className={[
              ...
              "text-sm font-semibold text-foreground hover:bg-surface-muted",
        ...
        <div className="mt-6 space-y-5">
          ...
          <div className="rounded-2xl border border-warning/40 bg-warning-surface p-4 text-warning-foreground">
            ...
            <p className="mt-1 text-xs leading-5 opacity-85">
              Restoring will fully replace {currentSessionCount} current {currentSessionCount === 1 ? "session" : "sessions"} with {selectedBackup.summary.sessionCount} backup {selectedBackup.summary.sessionCount === 1 ? "session" : "sessions"}.
            </p>
            <button
              type="button"
            ...
            >
              <RotateCcw aria-hidden="true" size={17} /> Restore Backup
            </button>
          </div>
        </div>
      ) : null}
      ...
      {isConfirmingRestore && selectedBackup ? (
        <div className="fixed inset-0 z-100 flex items-end justify-center bg-overlay p-4 sm:items-center">
          ...
          >
            ...
            >
              This permanently replaces the current local data. Export the
              current data first if it needs to be kept.
            </p>
