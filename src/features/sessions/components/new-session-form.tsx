...
export function NewSessionForm() {
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      console.error("Unable to create the SplitSukan session.", error);
      setCreationError(
        "The session could not be created. Your entered values are still available.",
      );
      setIsSubmitting(false);
    }
  }
      ...
      >
        <div className="max-w-2xl">
          ...
          <p className="mt-3 text-sm leading-6 text-muted-foreground sm:text-base">
            ...
            shared expenses can be added after the session is created.
          </p>
        </div>
                  ...
                  <input
                    ...
                    onChange={() => {
                      ...
                      if (option.value !== "other") {
                        setErrors((current) => {
                          const next = { ...current };
                          delete next.customActivityName;
                          return next;
                        });
                      }
                    }}
        ...
        <div className="mt-5">
          <label
            ...
            className="flex items-center gap-2 text-sm font-bold"
          >
            <NotebookText aria-hidden="true" size={19} />
            Note
        ...
        <dl className="mt-5 space-y-4">
          ...
          {values.note.trim() ? (
            <div>
              ...
              <dd className="mt-1 whitespace-pre-wrap wrap-break-word text-sm leading-6">
                {values.note.trim()}
              </dd>
            </div>
          ) : null}
        </dl>
        <div className="mt-6 rounded-2xl bg-success-surface p-4 text-success-foreground">
          <p className="text-sm font-semibold">Saved locally on creation</p>
